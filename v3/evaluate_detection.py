...
Naming follows spec 4.1 - bare `AP` is never used:
  ...
  mAP_full      all classes present (not just the frozen core), averaged over
                IoU - not spec-frozen, added only to compare against external
                10-class benchmarks (e.g. the BDD100K leaderboard). Never fed
                to acc_history; mAP@[.5:.95] stays the scalar the spec froze.
...
class DetectionEvaluator:
    ...
    def summarize(self):
        ...
        for name in self.class_names:
            ...
            # Recall per size band, not just AP. AP_S folds recall and
            # precision together, so a low AP_S cannot say whether the frozen
            # RPN never proposed the small object or the head proposed it and
            # scored it badly - which is the whole question behind "does the
            # RPN need training". AR_S answers it, and costs nothing: these
            # evaluations already run, the recall was simply discarded.
            ap_s, rec_s, n_s, _ = self._eval_class(name, AREA_RANGES["small"])
            ap_m, rec_m, n_m, _ = self._eval_class(name, AREA_RANGES["medium"])
            ap_l, rec_l, n_l, _ = self._eval_class(name, AREA_RANGES["large"])
            ...
            per_class[name] = {
                ...
                "AR_S": float(np.mean(rec_s)),
                "AR_M": float(np.mean(rec_m)),
                "AR_L": float(np.mean(rec_l)),
                # Band instance counts: an AP_S/AR_S without its N is not
                # reportable, same rule as N for the all-range metrics.
                "N_S": n_s,
                "N_M": n_m,
                "N_L": n_l,
                "P@0.5": prec,
        ...
        # All classes present, not just the frozen core - same presence rule
        # (N > 0, AP not NaN) applied to self.class_names instead of
        # self.frozen_core. This is the number comparable to external 10-class
        # benchmarks; it is not spec-frozen and never feeds acc_history.
        full_present = [c for c in self.class_names
                        if per_class[c]["N"] > 0 and not np.isnan(per_class[c]["AP@[.5:.95]"])]
        full_missing = [c for c in self.class_names if c not in full_present]

        def macro_full(key):
            vals = [per_class[c][key] for c in full_present]
            vals = [v for v in vals if not np.isnan(v)]
            return float(np.mean(vals)) if vals else float("nan")
        ...
        return {
            ...
            "mAP_full": macro_full("AP@[.5:.95]"),
            "mAP50_full": macro_full("AP50"),
            "mAR@100_full": macro_full("AR@100"),
            "per_class": per_class,
            "core_classes": core_present,
            "core_missing": missing,
            "core_complete": not missing,
            "full_classes": full_present,
            "full_missing": full_missing,
            "full_complete": not full_missing,
...
def evaluate_cross_client(client_models, pooled_loaders, device,
    ...
    Images-outer, models-inner: this is a 5x5 grid of (model, test-set) scores,
    same as reading each test set once per model - but each test set's images
    only need to be read and decoded from disk once, then handed to all 5
    models in turn, instead of re-reading it from scratch for every model.
    Loader order is fixed (pooled_test_loader shuffles=False), so every model
    sees each test set in the same order either way - the 5x5 numbers below are
    identical to the naive nested-loop version, just without redundant I/O.
    ...
    evaluators = {i: {j: DetectionEvaluator(class_names, frozen_core) for j in ids}
                  for i in ids}

    for j in ids:                                   # each test set read once
        for images, targets in pooled_loaders[j]:
            images = [img.to(device) for img in images]
            for i in ids:                            # every model scores it
                preds = client_models[i](images)
                evaluators[i][j].add_batch(preds, targets)

    matrix, own = {}, {}
    for i in ids:
        row = {}
        for j in ids:
            m = evaluators[i][j].summarize()
...
def print_cross_client(cross):
    # Matrix keys are already the client names (e.g. "clear", "rainy") - there
    # are no separate numeric ids in this codebase to map through.
    print("\n" + "=" * 60)
    print("CROSS-CLIENT TRANSFER (final models) -- HEADLINE")
    print("=" * 60)
    for i, row in cross["matrix"].items():
        print(f"Client {i} tested on:")
        for j, m in row.items():
            print(f"  {j:16} -> mAP@[.5:.95]: {m['mAP@[.5:.95]']:.4f}  "
                  f"mAP50: {m['mAP50']:.4f}")
