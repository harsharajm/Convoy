...
Every trainable parameter travels - "what you train, you share". In v2 that
was roi_heads alone (13.95M); in v3 the RPN trains too, so its ~594k weights
are on the wire as well (~14.55M, about +4%) and clients now average each
other's proposal generators, not just their classifiers. The frozen
ResNet+FPN still never travels: identical across clients, so aggregating it
would be a no-op that multiplies the message by ~3x. torchvision's detection
backbone uses FrozenBatchNorm2d, so no running statistics drift apart either.
...
def _trainable_state(model):
    """
    A detached *copy* of every parameter that travels: everything trainable.

    Named for the rule, not the module. In v2 this was roi_heads only and the
    name `_head_state` was accurate; v3 unfreezes the RPN, so its weights ride
    along too and the old name would have quietly lied about the payload.
    ...
    simultaneous exchange was order-dependent all along. ~14.55M floats, or
    ~56MB per snapshot: real, but small beside the activations.
    """
    return {name: p.detach().clone()
            for name, p in model.named_parameters() if p.requires_grad}


# Head losses only - unchanged from v2, despite the RPN now training.
#
# v3 first set this to all four terms on the argument "gate on what you
# train". gate_noise_probe.py measured that and it was wrong, at least under
# the conditions we can test. Comparing two never-gossiped clients (clear vs
# rainy, from local_only) on clear's own batch:
#
#     keys           excess    repeat-noise    signal/noise
...
# they dilute the head-loss difference rather than adding to it - the same
# argument that justified excluding them in v2, now with a number on it.
#
# Caveat: the probe loaded v2 checkpoints, whose RPNs are byte-identical, so
# it cannot say how much v3's RPNs diverge once trained. They train at
# LR_PRETRAINED (10x below the head) from a shared init, so probably little.
# Revisit once a v3 gossip run exists.
#
# The deciding argument is isolation: v3 already changes the detector. Leaving
# the gate signal exactly as v2 had it means a v3-vs-v2 gossip comparison
# moves one thing, not two.
GATE_LOSS_KEYS = ("loss_classifier", "loss_box_reg")


def gate_loss(model, batch, device):
    """
    The trainable model's training loss on a batch - the gate signal.
...
def _weighted_average(states, weights):
    """Per-parameter weighted average of several trainable state dicts."""
    keys = states[0].keys()
...
def mix_into(model, source_state, alpha):
    """model <- (1 - alpha) * model + alpha * source, trainable params only."""
    if alpha <= 0:
        return
...
def plan_gossip(client, neighbours, device, lam=GOSSIP_LAMBDA,
    ...
    Returns (record, aggregate). `aggregate` is the single state dict to mix
    ...
    for n in neighbours:
        ...
        states.append(_trainable_state(n["model"]))
        weights.append(w)
