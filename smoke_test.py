...
def main():
    ...
    check("backbone/FPN/RPN frozen, head trainable",
          0 < counts["trainable"] < counts["total"] * 0.5,
          f"{counts['trainable']:,} of {counts['total']:,} trainable")
    ...
    check("optimiser step moves the head",
          any(not torch.equal(b, a) for b, a in zip(before, after)))
